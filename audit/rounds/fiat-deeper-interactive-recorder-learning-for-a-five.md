...
## Step 1, round 4 -- 2026-08-31T19:10:52Z
...
Leads not pursued: All six round 1, four round 2 and two round 3 fixes were replayed in source and focused guards. Guide construction or resume failure returns to stopped state and promises only static note and fingering pictures, **Copy without sound** and grown-up recovery; maker-audio failure leaves the tune visible and playable by the child. **Finish without a tune** remains optional in the interface, README and ADR, and every route reaches participation-only completion. Guide playback enters a nullable pending state before `AudioContext.resume()`, uses the same 40 millisecond delay for sound and highlights, keeps stop available and copy or maker controls locked, and respects an immediate stop; the idle or stopped first note is a static lesson cue, not a scheduled moving pulse. Reduced-motion navigation selects immediate scrolling in the pure guard and the CSS disables motion. At 390 by 844, the active mission preceded the garden path, all rendered lesson controls measured at least 44 by 44 CSS pixels, lesson 12 fingering and its visible note advanced together, note stones stayed non-interactive outside microphone copy, no-microphone activities returned to the chooser, **Play to Pip** stopped at the ready card, and the no-tune route reached neutral stop, replay and garden choices. The exact Node 22.19.0 report executed 67 tests with no assertion failures, runner errors or skips; `verify:local`, dependency inspection and all three discipline lints passed, and production JavaScript measured 232827 bytes against the 300000-byte ceiling. No dependency, workflow, lesson, schema, generated catalogue, persisted field, recorder or outbound child-data path changed. Real-device audio and permission teardown, child observation, hosted publication and cache behaviour, assistive-technology behaviour and non-fixture provider responses remain external evidence boundaries.
