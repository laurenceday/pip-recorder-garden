## Step 1, round 1 -- 2026-08-31T20:32:13Z
...
Not checked: intended-child read-through; screen-reader and other assistive-technology runs; physical recorder, microphone, room, volume, comfort or learning outcome; Step 2 no-scroll, text-scale, orientation, safe-area and 64-pixel target criteria; Step 3 quiet route and progressive play flow; Step 4 public built-artifact boot. Independent browser evidence at 391 by 844 found child text Pip, B, Play and Back with no grown-up tree, but also 876 pixels of document height and 50-pixel actions; those geometry failures remain pending Step 2. The Pages setting remains legacy and requires separate user authority before Step 4 changes it.

Elenchus verdict: guarded

| id | severity | file | finding | status |
| --- | --- | --- | --- | --- |
| S1-R1-01 | high | scripts/check-child-copy.mjs | The claimed exhaustive gate inspected only ChildStage text and four fixed source digests. Visible Recorder injected through imported GardenMark, CSS generated content, placeholder, value, children or aria-description passed, and a fake childMode conditional with no child return also passed. | fixed in this commit |
| S1-R1-02 | medium | scripts/check-child-copy.mjs | A report accepted dirty inspected source bytes while naming the unchanged HEAD commit, so its commit field did not bind the checked surface. | fixed in this commit |
...
## Step 4, round 2 -- 2026-09-04T11:54:00Z

Audit schema: fiat-audit-round/v2

Covered: child-copy-leak=reviewed; phonics-overclaim=reviewed; parent-mode-leak=reviewed; child-overload=reviewed; mobile-scroll=reviewed; text-scale-crop=reviewed; quiet-audio-leak=reviewed; pattern-model-drift=reviewed; audio-overlap=reviewed; screen-fingering-transfer=reviewed; false-learning-signal=reviewed; attention-capture=reviewed; progress-pressure=reviewed; privacy-expansion=reviewed; agent-copy-bypass=reviewed; deployment-overwrite=reviewed; bundle-growth=reviewed

Not checked: intended-child read-through; screen-reader and other assistive-technology runs; physical phone or tablet browser chrome; physical recorder, microphone, room, volume, comfort or learning outcome; live boot of the accepted `main` tree, which remains an integration gate. The checker paths, redirects, response-size limits, input bindings, report destination and fetch lifecycle were inspected.
...
## Integration availability repair -- 2026-09-04T11:23:35Z

The next hosted release passed all 104 tests, type, lint, source-boundary and production-build checks, then failed after the npm advisory endpoint returned HTTP 503 for seven minutes. A bounded workspace probe reproduced the unavailable audit service. Pages now installs the locked graph without npm's implicit advisory call and runs the deterministic artifact gate; the explicit advisory audit remains fail-closed in `verify:local` and ordinary CI. A workflow contract test guards that separation and raises the suite to 105 tests. No application source, dependency or production byte changed.
