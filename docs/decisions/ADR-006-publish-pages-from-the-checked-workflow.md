...
GitHub Actions owns Pages publication. The repository Pages setting uses workflow build type. The workflow checks the accepted `main` tree, uploads only `dist/` and grants deployment authority only to its deploy job.
